...
//    require(mHome != null, "MAHOUT_HOME is required to to run Mahout with VCL")
    ...
    val vclLibrary = new File(mHome,"/viennacl/target/classes/org/apache/mahout/javacpp/linalg/linux-x86_64/libjnivcl_blas3.so")

    // set the value of useVCL to the existance of the compiled library on the above hardcoded path.
    vclLibrary.exists()
  }
