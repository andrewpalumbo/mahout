 ...
 * @author gokhan
 */
class DrmLikeIOSuite extends FunSuite with DistributedSparkSuite  {
  test("seq2sparse") {
    ...
    matrixAndDictTFIDFUni._1.keyClassTag should be (ClassTag.Int)
  ...
  private def createTextCollection(tmpDir: String): Path = {
   ...
   // seqWriter.append(new IntWritable(2), new Text(""))
  ...
  test("seq2sparse Text-Keyed") {
    val textCollection = createTextKeyedTextCollection(TmpDir)
    val fileName = textCollection.toUri.getPath
    ...
    matrixAndDictTFIDFUni._1.keyClassTag should be (ClassTag(classOf[String]))
    ...
    val rowLabelMap = tfUni.getRowLabelBindings.asScala

    rowLabelMap("/Category0/doc_1") should be (0)
    rowLabelMap("/Category1/doc_1") should be (1)
    rowLabelMap("/Category2/doc_1") should be (2)
    rowLabelMap("/Category3/doc_1") should be (3)
  ...
  private def createTextKeyedTextCollection(tmpDir: String): Path = {
    val conf = new Configuration()
    ...
    seqWriter.append(new Text("/Category0/doc_1"), new Text("Word1 word2 word3 word2"))
    seqWriter.append(new Text("/Category1/doc_1"), new Text("Word1 word4 word3 word4"))
    seqWriter.append(new Text("/Category2/doc_1"), new Text(""))
    seqWriter.append(new Text("/Category3/doc_1"), new Text("Word1 word2 word3 word4"))
    seqWriter.close()
